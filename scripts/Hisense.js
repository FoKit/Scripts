...
let HISENSE_GAME_SCORE = ($.isNode() ? process.env.HISENSE_GAME_SCORE : $.getdata(HISENSE_GAME_SCORE_KEY)) || '500-700';
    ...
    $.get(opt, async (err, resp, data) => {
      try {
        ...
        if (data) {
          ...
          console.log(`开始第 ${k} 次[打地鼠]游戏: ${$.gameCode}...`);
...
async function submitScore() {
  ...
  $.gameScore = randomNumber(ScoreArr[0], ScoreArr[1]) * 10;
  ...
  return new Promise(resolve => {
    $.post(opt, async (err, resp, data) => {
      try {
        ...
        let result = JSON.parse(data);
        if (result) {
          if (result?.isSuccess && data?.resultCode == "00000") {
            message += `打地鼠游戏, 获得 ${result.data.obtainScore} 积分 🎉\n`;
          } else {
